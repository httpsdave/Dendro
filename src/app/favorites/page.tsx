...
import { Heart, Leaf, Trash2, ExternalLink, Search } from 'lucide-react';
import Image from 'next/image';
import { useDendroStore } from '@/store';

export default function FavoritesPage() {
  const { favorites, removeFavorite } = useDendroStore();
  const [search, setSearch] = useState('');

  const filtered = favorites.filter(
    (f) =>
      f.plantName.toLowerCase().includes(search.toLowerCase()) ||
      (f.plantScientificName ?? '').toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <p className="text-forest-600">
            {favorites.length} plant{favorites.length !== 1 ? 's' : ''} saved to your favorites
          </p>
        </motion.div>
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            <AnimatePresence mode="popLayout">
              {filtered.map((fav) => (
                ...
                >
                  {/* Image */}
                  <Link href={`/plants/${fav.plantId}`} className="block relative h-44 overflow-hidden">
                    {fav.plantImage ? (
                      <Image
                        src={fav.plantImage}
                        alt={fav.plantName}
          ...
          <div className="text-center py-20">
            ...
            <p className="text-forest-500 text-sm">Try adjusting your search term.</p>
          </div>
        ) : (
          <div className="text-center py-20">
            <Heart className="w-16 h-16 text-forest-200 mx-auto mb-4" />
            <h3 className="text-lg font-display font-bold text-forest-900 mb-2">No Favorites Yet</h3>
            <p className="text-forest-500 mb-6 text-sm">
              Start exploring plants and tap the heart icon to save your favorites.
            </p>
            <Link
              href="/explore"
